...
public class ImageErrorController extends BaseController {
	...
	/**
	 * Handle Upload Fail Exception
	 * @param uploadFailException
	 * @param request
	 * @return
	 */
	@ExceptionHandler(UploadFailException.class)
    @ResponseBody
    protected ResponseEntity<APIResponse<String>> handleUploadFailException(
    		final UploadFailException uploadFailException, final HttpServletRequest request) {
		logger.error("handle UploadFailException");
        return ApiHelper.<String>createAndSendErrorResponseWithHeader(
        		ImageResponseCode.FAILED_TO_UPLOAD_IMAGE, HttpStatus.INTERNAL_SERVER_ERROR,
    ...
    @ResponseBody
    protected ResponseEntity<APIResponse<String>> handleMultipartException(
    		final MultipartException multipartException, final HttpServletRequest request) {
